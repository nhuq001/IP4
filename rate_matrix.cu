__global__ rate_matrix(int *config, int *rate, species *all, )//species is a struct that holds reaction and diffusion rates of a species
{
	...
	if(element % 3 == 1) //first column
		rate[element] = sum_reactions(element, config, species); //sums reaction rate for subvolume x. Don't know how to find x yet in config
	if(element % 3 == 2) //second column
		rate[element] = sum_diffusions(element, config, species); //sums diffusion rate for subvolume x. Don't know how to find x yet in config
